<template>
    <div class="image-card image-card-width">
        ...
        >
            <button
                class="listing-card-save-button"
                type="button"
            >
                <svg
                    viewBox="0 0 32 32"
            ...
            <div class="wrapper-for-nav-indicator-carousel">
                <div class="nav-indicator-carousel-window">
                    <div class="nav-indicator-carousel-track">
                        ...
                        <div
                            v-for="image in imageLocationArray"
                            class="nav-indicator-carousel-item"
                            :id="image.id"
                            :key="image.id"
                        ></div>
                    </div>
...
export default {
    ...
    mounted() {
        ...
        const navCarouselTrack = carouselImagesSets.querySelector(
            ".nav-indicator-carousel-track"
        );
        const navCarouselItems = carouselImagesSets.querySelectorAll(
        ...
        navCarouselTrack.style.width = `${
            navCarouselItemCount * carouselItemWidth
        }px`;
    ...
    methods: {
        handleScrolling() {
            ...
            const navCarouselItems =
                this.$refs.imageCarouselContainer.querySelectorAll(
                    ".nav-indicator-carousel-item"
                );
            const navCarouselTrack =
                this.$refs.imageCarouselContainer.querySelector(
                    ".nav-indicator-carousel-track"
                );
            const navCarouselItemCount = navCarouselTrack.children.length;

            // Get the width of the carousel item, including margins
            const carouselItemWidth = this.getFullWidth(
                carouselImages.children[1]
            );
            ...
            if (currentImageIndex !== this.currentImageIndex) {
                ...
                if (this.currentImageIndex < 2) {
                    ...
                    navCarouselTrack.style.transform = `translateX(-${
                        (navCarouselItemCount - 5) * navCarouselItemWidth
                    }px)`;
                } else {
                    navCarouselTrack.style.transform = `translateX(-${
                        (this.currentImageIndex - 2) * navCarouselItemWidth
                    }px)`;
                }
                ...
                if (currentImageIndex < 3) {
                    ...
                    navCarouselItems[navCarouselItemCount - 4].classList.add(
                        "near-edge"
                    );
                    navCarouselItems[navCarouselItemCount - 5].classList.add(
                        "edge"
                    );
                } else {
                    navCarouselItems[currentImageIndex + 1].classList.add(
                        "near-edge"
                    );
                    navCarouselItems[currentImageIndex + 2].classList.add(
                        "edge"
                    );
                    navCarouselItems[currentImageIndex - 1].classList.add(
                        "near-edge"
        ...
        onWindowResize() {
            ...
            const alignedPosition =
                Math.round(carouselImages.scrollLeft / imageCarouselItemWidth) *
                imageCarouselItemWidth;
        ...
        async performNextScroll(e) {
            ...
            imageCarouselContainer = e.target.closest(
                ".image-carousel-container"
            );
            const carouselImages =
                imageCarouselContainer.querySelector(".carousel-images");
            let imageCount;
            imageCount = carouselImages.children.length;

            // Get the width of the images
            const imageCarouselItemWidth = this.getFullWidth(
                carouselImages.children[1]
            );

            // Set the width of the carousel track to number of * width of the .nav-indicator-carousel-item
            const navCarouselTrack = imageCarouselContainer.querySelector(
                ".nav-indicator-carousel-track"
            );
            const navCarouselItems = imageCarouselContainer.querySelectorAll(
                ".nav-indicator-carousel-item"
            );
            const carouselItemWidth = this.getFullWidth(navCarouselItems[1]);
            ...
            navCarouselItems[this.currentImageIndex - 1].classList.remove(
                "active"
            );
            navCarouselItems[this.currentImageIndex].classList.remove(
                "edge",
                "near-edge"
            );
            ...
            if (
                ...
                this.currentImageIndex < imageCount - 3
            ) {
                navCarouselTrack.style.transform = `translateX(-${
                    (this.currentImageIndex - 2) * carouselItemWidth
                }px)`;
                navCarouselItems[this.currentImageIndex + 2].classList.add(
                    "edge"
                );
                navCarouselItems[this.currentImageIndex + 1].classList.remove(
                    "edge",
                ...
                navCarouselItems[this.currentImageIndex + 2].classList.remove(
                    "edge",
                    "near-edge"
                );
                navCarouselItems[this.currentImageIndex + 1].classList.remove(
                ...
                navCarouselItems[this.currentImageIndex - 1].classList.add(
                    "near-edge"
                );
                navCarouselItems[this.currentImageIndex - 2].classList.add(
                    "edge"
        ...
        async performPrevScroll(e) {
            ...
            if (
                this.currentImageIndex > 2 &&
                this.currentImageIndex < imageCount - 2
            ) {
                navCarouselTrack.style.transform = `translateX(-${
                    (this.currentImageIndex - 2) * carouselItemWidth
                }px)`;
